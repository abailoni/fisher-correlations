...
cdef:
    interpolation_tools zero_spectrum_tools[2]
    interpolation_tools derivatives_tools[N_vars]
    interpolation_tools *tools
    interpolation_tools zero_spectrum_tools_anorm[2]
...
cdef double zero_spectrum(double k):
    return eval_interp_GSL(k, &zero_spectrum_tools[0])
...
cdef double zero_spectrum_der_k(double k):
    return gsl_spline_eval_deriv(zero_spectrum_tools[0].spline, k, zero_spectrum_tools[0].acc)

cdef double CAMB_numerical_paramDER(double k, int num_var): # Var goes from 1 to 4
    return (eval_interp_GSL(k, &derivatives_tools[num_var]) - zero_spectrum(k)) / (ref_val_v[num_var])
