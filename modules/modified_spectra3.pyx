# 08/03/2016 -- Branch
#
# Old (not working) version for integral_23 with integrals in z divided
...
# from libc.math cimport M_PI as ciaoooone
cimport numpy as np
...
def integral_1_DEF(int num_var, int bin1, int bin2):
    new_k_min=k_min
    new_k_max=0.5
...
cdef double arg_integral_2(double kp, void *inputs): #k, bin1, bin2, num_var
    ...
    # if ( abs(k-kp)<1e-1):
    integral = eval_integration_GSL(z_min[0], 0.999, abs_prec_z_23,rel_prec_z_23, params_int, W_z, &F_z, MAX_ALLOC)
        # Increasing this precision gives an error:
    integral = integral + eval_integration_GSL(0.999, 0.9999999999999, 1e-5, 1e-4, params_int, W_z, &F_z, MAX_ALLOC)
    # else:
...
def integral_DER_K_DEF(int num_var, int bin1, int bin2):
    gsl_set_error_handler_off()
    new_k_min = k_min
    z_min[0] = -1.
    z_max[0] = 1.
    ...
    vect_k = np.concatenate((np.linspace(new_k_min,0.2,130), np.linspace(0.2,0.5,50)[1:]))
    integral2, integral3 = np.empty(vect_k.shape[0]), np.empty(vect_k.shape[0])
