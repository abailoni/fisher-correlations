...
for i in range(N_k):
    convSpec_1[i] = cFM.growth_spectrum_py(0,2,k_vect[i],"windowFun")
    convSpec_2[i] = cFM.growth_spectrum_py(0,6,k_vect[i],"windowFun")
    convSpec_3[i] = cFM.growth_spectrum_py(0,13,k_vect[i],"windowFun")
    # spectrum[i] = cFM.growth_spectrum_py(bin1,bin2,k_vect[i],"uncorr")


plot_fcts(k_vect[:-80],[convSpec_1[:-80],convSpec_2[:-80],convSpec_3[:-80]], "compare_corr", ["$P_{\\mathrm{conv}}(k;z_1,z_3)$", "$P_{\\mathrm{conv}}(k;z_1,z_7)$", "$P_{\\mathrm{conv}}(k;z_1,z_{14})$"])
