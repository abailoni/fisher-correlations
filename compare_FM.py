...
INPUT_FOLDER = "OUTPUT/FFT/"
OUTPUT_FOLDER = "plots/marginalisation/comparison/FFT/EUCLID2016/"
name = "EUCLID2016_AP1-14bins"
W = 1
cFM = np.loadtxt(open(INPUT_FOLDER+"FMcorr_"+name+".csv","rb"),delimiter=" ",skiprows=0)
uFM = np.loadtxt(open(INPUT_FOLDER+"FMuncorr_"+name+"-W%d.csv" %(W),"rb"),delimiter=" ",skiprows=0)
...
def print_ellipse(cFM,uFM,var1,var2): #var names!
    if n_var[var1]>n_var[var2]:
        vartemp = var1
        var1 = var2
        var2 = vartemp
    results1 = sigma1_ellipse(cFM,n_var[var1],n_var[var2])
    results2 = sigma1_ellipse(uFM,n_var[var1],n_var[var2])
    print results2[0], results2[1]
    print degrees(results1[2]), degrees(results2[2])
    ellipse1 = patch.Ellipse(xy=(ref_values[var1], ref_values[var2]), width=results1[0], height=results1[1], angle=degrees(results1[2]), alpha= 0.3, color="g", label="Correlations + W. F.")
    # ellipse2 = patch.Ellipse(xy=(0,0), width=0.03, height=0.002, angle=46)
    if W==0:
        label2 = "Uncorrelated"
    else:
        label2 = "Window function"
    ellipse2 = patch.Ellipse(xy=(ref_values[var1], ref_values[var2]), width=results2[0], height=results2[1], angle=degrees(results2[2]), alpha= 0.3, color="b", label=label2)
    fig=pl.figure()
    ...
    fig.savefig(OUTPUT_FOLDER+'correlations_'+name+'-W%d_vars_%d_%d.pdf' %(W,n_var[var1],n_var[var2]))
...
print_ellipse(cFM,uFM,var1,var2)
